...
def main():
    ...
    validation_split_list = config["dataset"]["validation_split_list"]
    num_models = len(validation_split_list)
    dataset_size_list = []
    vae_r2_traing_list = []
    vae_r2_test_list = []
    mlp_r2_traing_list = []
    ...
    for i in tqdm(range(0, num_models)):
        ...
        dataset_size_list.append(len(train_data))

        print(f' ---------------------- dataset size: {len(train_data)} ---------------------- ', end='\\')
        ...
        epochs_no_improve = 0
        patience_VAE = config["training_parameters"]["patience_VAE"]
        ...
        for epoch in range(0, epochs):
            ...
            if valid_loss < best_valid_loss:
                ...
                epochs_no_improve = 0
                torch.save(vae.state_dict(), os.path.join(log_dir, config['model_name']['VAE_model_prefix'] + str(len(train_data)) + '.pt'))
            else:
                epochs_no_improve += 1
        ...
        # Save VAE
        model_path = os.path.join(model_dir, config['model_name']['VAE_model_prefix'] + str(len(train_data)) + '.pt')
        ...
        if config['model_name']['combined_load_pretrained']:
            model_path = os.path.join(model_dir, config['model_name']['combined_model_prefix'] + str(len(train_data)) + '.pt')
            print(model_path)
        ...
        # Save MLP
        model_path = os.path.join(log_dir, config['model_name']['combined_model_prefix'] + str(len(train_data)) + '.pt')
        print('model path: ', model_path)
    ...
    axs[0].plot(dataset_size_list, vae_r2_test_list)
    axs[1].plot(dataset_size_list, mlp_r2_test_list)
    axs[0].set_title('VAE')
